...
import AnnualView from './components/budget/AnnualView'; // <-- IMPORT
import { LayoutDashboard, Settings, LogOut, Loader2, PiggyBank } from 'lucide-react'; // <-- ICONE
    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-slate-100">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center max-w-md w-full">
          ...
          <button onClick={login} className="w-full bg-blue-600 text-white py-3 rounded-lg font-bold hover:bg-blue-700 transition">Se connecter avec Google</button>
        </div>
      </div>
    );
  }
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between overflow-x-auto">
          <div className="flex items-center gap-4 min-w-fit">
             ...
             <h1 className="text-lg font-bold hidden md:block">Budget de {user.displayName}</h1>
          </div>
          <div className="flex gap-2">
            <NavButton active={view === 'month'} onClick={() => setView('month')} icon={LayoutDashboard} label="Mensuel" />
            <NavButton active={view === 'annual'} onClick={() => setView('annual')} icon={PiggyBank} label="Provisions" />
...
const NavButton = ({ active, onClick, icon: Icon, label }) => (
  <button 
    onClick={onClick} 
    className={`px-3 py-2 rounded-lg flex items-center gap-2 transition-colors text-sm font-medium ${active ? 'bg-blue-50 text-blue-700' : 'text-slate-500 hover:bg-slate-50'}`}
  >
